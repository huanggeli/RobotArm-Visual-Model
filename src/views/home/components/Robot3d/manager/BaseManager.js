...
export default class baseManager {
    constructor(canvas) {
        ...
        this.gui.hide();//隐藏调试gui，需要调试的时候打开
        //Canvas
        this.canvas = canvas
        //Sizes
        this.sizes = {}
    ...
    initRobot() {
        const D1 = new THREE.Mesh(
            new THREE.CylinderGeometry(1, 1, 0.5, 32),//创建圆柱体
        ...
        this.gui.add(D1.rotation, "y").min(-Math.PI).max(Math.PI).step(0.01);//第一个关节

        const D2 = new THREE.Mesh(
            new THREE.SphereGeometry(0.5, 32, 16),//创建球体
            new THREE.MeshStandardMaterial({
                color: "#1B1A17",
                metalness: 0,
                roughness: 0.5,
        ...
        D3.position.set(0, 1.5, 0);//(红色，绿色，蓝色)（y,z,x）
        ...
        //初始化机械臂角度
        D2.rotation.z = 80*Math.PI / 180;
        D3.rotation.z = -160*Math.PI / 180;
        D4.rotation.z = -100*Math.PI / 180;
